defmodule S3Test do
  ...
  test "ensure build/1 preserves port" do
    config = fn options -> Keyword.merge(@config, options) end

    assert {%URI{port: 80}, _headers, _body} =
             S3.build(config.(method: :head, url: "http://example.com"))

    assert {%URI{port: 443}, _headers, _body} =
             S3.build(config.(method: :head, url: "https://example.com"))

    assert {%URI{port: 8080}, _headers, _body} =
             S3.build(config.(method: :head, url: "http://example.com:8080"))

    assert {%URI{port: 8443}, _headers, _body} =
             S3.build(config.(method: :head, url: "https://example.com:8443"))
  end
end
